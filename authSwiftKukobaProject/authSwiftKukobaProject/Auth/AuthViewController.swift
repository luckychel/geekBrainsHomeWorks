...
class AuthViewController: UIViewController {
    ...
    //MARK: Авторизация пользователя по кнопке Войти
    @IBAction func loginBtn(_ sender: Any) {
        ...
        service.signUser(email: login, password: password) { res in
            if res.isSuccess {
                UserDefaults.standard.set(true, forKey: "isAuth")
                self.performSegue(withIdentifier: "showPostsFromAuth", sender: nil)
            }
            else {
